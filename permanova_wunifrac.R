...
dist <- read.table("data/TV47_Fig4/TV47_weighted_unifrac.txt")
...
# export table
write.table(permanova.result, file="results/permanova_weighted_unifrac.txt", sep="\t", row.names=F, quote=F)
write.table(cap.result, file="results/cap_weighted_unifrac.txt", sep="\t", row.names=F, quote=F)